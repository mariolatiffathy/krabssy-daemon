...
def server_power(server_id):
    ...
    try:
        subprocess.check_output(("screen -S " + SERVER_CONTAINER_ID + " -X select . ; echo $?").split(" "), shell=True, cwd="/home/fabitmanage/daemon-data/" + SERVER_CONTAINER_ID, preexec_fn=AsUser(int(SERVER_CONTAINER_UID), int(SERVER_CONTAINER_GID)))
    ...
    if req_data['action'] == "start":
        if SCREEN_SESSION_EXISTS == False:
            ...
            subprocess.check_output(['screen', '-d', '-m', '-S', SERVER_CONTAINER_ID], shell=True, cwd="/home/fabitmanage/daemon-data/" + SERVER_CONTAINER_ID, preexec_fn=AsUser(int(SERVER_CONTAINER_UID), int(SERVER_CONTAINER_GID)))
            subprocess.check_output(("screen -S " + SERVER_CONTAINER_ID + " -X stuff '" + SERVER_STARTUP_COMMAND + "'$(echo -ne '\015')").split(" "), shell=True, cwd="/home/fabitmanage/daemon-data/" + SERVER_CONTAINER_ID, preexec_fn=AsUser(int(SERVER_CONTAINER_UID), int(SERVER_CONTAINER_GID)))
    ...
    if req_data['action'] == "stop":
        if SCREEN_SESSION_EXISTS == True:
            ...
            subprocess.check_output(['screen', '-S', SERVER_CONTAINER_ID, '-X', 'quit'], shell=True, cwd="/home/fabitmanage/daemon-data/" + SERVER_CONTAINER_ID, preexec_fn=AsUser(int(SERVER_CONTAINER_UID), int(SERVER_CONTAINER_GID)))
    ...
    while True:
        ...
        if queue_cursor.rowcount > 0:
           ...
           if queue_action == "delete_server":
               ...
               try:
                   subprocess.check_output(['screen', '-S', CONTAINER_ID, '-X', 'quit'], shell=True, cwd="/home/fabitmanage/daemon-data/" + CONTAINER_ID, preexec_fn=AsUser(int(CONTAINER_UID), int(CONTAINER_GID)))
               except Exception as e:
                   pass
